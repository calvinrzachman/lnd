...
func testSendOnion(ht *lntest.HarnessTest) {
	...
	// We'll open several channels between Alice and Bob with Alice's
	// liquidity drained (eg: all funds on Bob's side of the channel) to
	// exercise the RPC logic which selects the appropriate channel link.
	...
	chanPointBobAlice2 := ht.OpenChannel(
		bob, alice, lntest.OpenChannelParams{Amt: chanAmt},
	)
	defer ht.CloseChannel(alice, chanPointBobAlice2)
