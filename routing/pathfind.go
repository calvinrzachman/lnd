...
type RestrictParams struct {
	...
	// HopFeatures is a feature vector describing the features each hop
	// in a route is required to support. An example use is requiring that
	// every hop in a blinded route advertise support for the route blinding
	// feature bit.
...
func findPath(g *graphParams, r *RestrictParams, cfg *PathFindingConfig,
	...
	processEdge := func(fromVertex route.Vertex,
		...
		// This function will return successfully/no-op if
		// we do not have any required path features.
		if !fromFeatures.HasFeatures(desiredHopFeatures) {
			log.Tracef("ignoring edge from (%s)...", fromVertex)
