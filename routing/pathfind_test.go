...
func runMultiOrigin(t *testing.T, useCache bool) {
	...
	// When the target is also in the origin set, the pathfinder
	// may find either a cross-gateway path (gw1→alice→dest) or a
	// circular route through dest, depending on heap tiebreaks at
	// equal cost. We only assert that a valid path is found from
	// one of the origins. The runMultiOriginCircularRoute test
	// exercises deterministic circular-vs-cross-gateway selection
	// with an explicit topology.
	...
	require.True(t, targetIsOrigin.IsOrigin(source),
		"source should be one of the origins")
}
