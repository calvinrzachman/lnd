...
const (
	...
	// BlindingPointOnionType is the type used in the top level onion TLV
	// payload to refer to an ephemeral public key chosen by the
	// constructor of a blinded route (ex: recipient) for the first hop
	// in a blinded route. This is delivered in the TLV onion payload for
	// the first hop ("introduction node") of a blinded route ONLY.
...
// (type 12) for an onion payload. This is a public key chosen by the constructor
// of a blinded route for the first hop in a blinded route...
