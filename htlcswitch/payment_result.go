...
func (store *networkResultStore) InitAttempt(attemptID uint64) error {
	...
	//
	// TODO(calvin): When migrating to native SQL storage, replace this
	// custom message placeholder with a proper status enum/struct to
	...
	// Mark an attempt with this ID as having been seen by storing the
...
func (store *networkResultStore) notifySubscribers(attemptID uint64,
	...
	// We can notify any active subscribers - but only if this isn't an
	// initialized attempt awaiting a settle/fail result from the network.
