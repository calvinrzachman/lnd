...
func (l *channelLink) handleDownstreamPkt(pkt *htlcPacket) {
	...
	case *lnwire.UpdateFailHTLC:
		...
		// The incoming link routinely responds directly (ie: not via this
		// code path) to peers for local errors. Will we need a way to
...
func (l *channelLink) processBlindHop(pd *lnwallet.PaymentDescriptor,
	...
	ValidatePathID := func() error {
		if blindHopPayload.PathID != nil {
			// We'll verify that the path ID data matches what we expect.
			pathIDHash := sha256.Sum256(blindHopPayload.PathID)
			l.log.Debugf("sha-256(path_id)=%v, payment hash=%v",
				pathIDHash, pd.RHash,
			)

			// We assume for now that any blinded routes we (LND) create
			// willstore the preimage in the path_id field of the route
			// blinding payload for the final hop. In the future we might
			// expand the information stored in the path_id so that we can
			// properly handle dummy hops.
			// Also, if we did not generate the blinded route, and the path
			// ID is an actual pre-image for the payment then we just happily
			// accept the free money!
			if !bytes.Equal(pathIDHash[:], pd.RHash[:]) {
				l.log.Errorf("unexpected path_id=%v for payment, expected=%v",
					pathIDHash, pd.RHash,
				)
