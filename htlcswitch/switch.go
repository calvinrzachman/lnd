...
type Config struct {
	...
	// NetworkDir is the directory where the channel.db file is located.
	NetworkDir string
}
...
func New(cfg Config, currentHeight uint32) (*Switch, error) {
	...
	markerFilePath := filepath.Join(cfg.NetworkDir, "remote_router.marker")

	// If this is typical deployment using the on-board router, check for
	// the existence of the marker file. If it exists, we must exit to
	// prevent data loss. This avoids lnd's onboard channel router from
	// clearing state needed to track payment life-cycle for payments that
	// it did not inititate.
	if !cfg.RemoteRouter {
		if _, err := os.Stat(markerFilePath); err == nil {
			return nil, fmt.Errorf("HTLC attempt database marked "+
				"for remote management, but this lnd build is "+
				"in local mode. Halting to prevent data loss. "+
				"To use this database, either restart with "+
				"the `switchrpc` build tag, or manually "+
				"delete the marker file (if you are sure "+
				"there are no in-flight payments): %v",
				markerFilePath)
		}
	}

	// If we are using the switchrpc server for htlc dispatch, create the
	// marker file. This protects attempt state generated by the external
	// router, which would otherwise be cleaned by the on-board router
	// during startup.
	if cfg.RemoteRouter {
		// We'll create the file if it doesn't exist. We don't need to
		// write anything to it.
		f, err := os.OpenFile(
			markerFilePath, os.O_CREATE|os.O_EXCL|os.O_WRONLY, 0644,
		)
		if err != nil && !os.IsExist(err) {
			return nil, fmt.Errorf("unable to write remote "+
				"router marker file: %w", err)
		}
		if f != nil {
			err := f.Close()
			if err != nil {
