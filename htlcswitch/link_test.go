...
func testLinkEnforcesBOLT04(t *testing.T) {
	...
	testCases := []RouteBlindingTest{
		...
		// 	// rather than a as a table driven test.
		...
		// },
		{
			name: "blind hop with blinding point in both msg and tlv payload", // error case (top level)
			onionPayload: &hop.Payload{
				BlindingPoint: ephemeralBlindingPoint,
			},
			routeBlindingPayload: &hop.BlindHopPayload{
		...
		{
			name:         "blind hop missing blinding point in msg and tlv payload", // error case (top level)
			onionPayload: &hop.Payload{},
			routeBlindingPayload: &hop.BlindHopPayload{
				NextHop:      n.carolChannelLink.ShortChanID(),
				PaymentRelay: computePaymentRelay(n.secondBobChannelLink),
		...
		// },
		// {
		// 	name: "blind hop with AMP improperly set in top level TLV payload", // error case (cross onion + RB payload)
		// 	onionPayload: &hop.Payload{
		// 		AMP: &record.AMP{},
		// 	},
		// 	routeBlindingPayload: &hop.BlindHopPayload{
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			// delivered to Bob.
			var b bytes.Buffer
			test.routeBlindingPayload.PathID = preimage[:] // use the preimage as the path_id
			...
			// behaves as an intermediate processing node (how about introduction node?).
			...
			// Otherwise, we'll test Bob's link as a final hop.
			// Add the invoice to Bob's invoice registry so that he's expecting payment.
			if test.isFinalHop {
				err = n.bobServer.registry.AddInvoice(*invoice, test.htlc.PaymentHash)
...
var validateOnionPayloadTests = []validateOnionPayloadTest{
	...
	// NOTE(11/16/22): Are we double dipping with validation testing
	// done at payload parse time here?
	{
		name: "blind hop with MPP improperly set in top level TLV payload", // error case (cross onion + RB payload)
		parsedPayload: &hop.Payload{
			RouteBlindingEncryptedData: []byte{},
	...
	// },
	{
		name: "blind hop with total_amount_msat improperly set in top level TLV payload", // error case (cross onion + RB payload)
		parsedPayload: &hop.Payload{
			RouteBlindingEncryptedData: []byte{},
